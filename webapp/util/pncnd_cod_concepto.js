sap.ui.define([
    ...
    var COLS_NO_CLAVE = [
        { field: "descripcion", label: "Descripción", maxLength: 40, hint: "Descripción del concepto" }
    ];
    ...
    p.onEditar = function (oEvent) {
        ...
        this._getDlgEditar().then(function () {
            ...
            Fragment.byId(sId, "eConcCodConcepto").setValue(oData.cod_concepto);
            Fragment.byId(sId, "eConcDescripcion").setValue(oData.descripcion || "");
            that._oDlgEditar.open();
        });
    };

    p.onConfirmarEditar = function () {
        var sDescripcion = Fragment.byId(this._sDlgEditarId, "eConcDescripcion").getValue().trim();
        var that = this;
        MessageBox.confirm("¿Confirma la Edición del Registro?", {
            title  : "Confirmar Edición",
            onClose: function (sAction) {
                if (sAction !== MessageBox.Action.OK) { return; }
                that._model().update(that._sEditPath, { descripcion: sDescripcion }, {
                    success: function () {
            ...
            oReader.onload = function (e) {
                try {
                    ...
                    aData.forEach(function (oReg, idx) {
                        if (!oReg.cod_concepto) {
                            aErrors.push("Fila " + (idx + 3) + ": campo \"Cod. Concepto\" es requerido");
                        }
                    });
                    if (aErrors.length) {
    ...
    p._getDlgEditar = function () {
        ...
        var sId = this.c.createId("conc_dlgEdit");
        this._sDlgEditarId = sId;
        return Fragment.load({ id: sId, controller: this.c,
            name: "zpncnd.datos.maestros.pncnddatosmaestros.view.fragment.DialogEditarCodConcepto"
    ...
    p._getDlgMM = function () {
        ...
        var sId = this.c.createId("conc_dlgMM");
        this._sDlgMMId = sId;
        return Fragment.load({ id: sId, controller: this.c,
    ...
    p._getDlgCM = function () {
        ...
        var sId = this.c.createId("conc_dlgCM");
        this._sDlgCMId = sId;
        return Fragment.load({ id: sId, controller: this.c,
    ...
    p._getDlgCP = function () {
        ...
        var sId = this.c.createId("conc_dlgCP");
        this._sDlgCPId = sId;
        return Fragment.load({ id: sId, controller: this.c,
