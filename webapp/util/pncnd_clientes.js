sap.ui.define([
    ...
    var ENTITY_SET = "PNCND_CLIENTES";
    var NOMBRE     = "Clientes";
    var ST_ID      = "stClientes";

    var COLS_TEMPLATE = [
        { label: "Nro. Cliente", hint: "Texto - máx. 10 caracteres" },
        { label: "Org. Ventas", hint: "Texto - máx. 4 caracteres"  },
        { label: "Canal",       hint: "Texto - máx. 2 caracteres"  },
        { label: "Sector",      hint: "Texto - máx. 2 caracteres"  },
        { label: "Of. Ventas",  hint: "Texto - máx. 4 caracteres"  },
        { label: "División",    hint: "Texto - máx. 10 caracteres" },
        { label: "Nombre",      hint: "Texto - máx. 35 caracteres" }
    ];
    ...
    var COLS_NO_CLAVE = [
        { field: "vkorg", label: "Org. Ventas", maxLength: 4,  hint: "Ej: 1000"           },
        { field: "vtweg", label: "Canal",        maxLength: 2,  hint: "Ej: 10"             },
        { field: "spart", label: "Sector",       maxLength: 2,  hint: "Ej: 00"             },
        { field: "vkbur", label: "Of. Ventas",   maxLength: 4,  hint: "Ej: 1001"           },
        { field: "bran2", label: "División",     maxLength: 10, hint: "Ej: LUBRICANTES"    },
        { field: "name1", label: "Nombre",       maxLength: 35, hint: "Nombre del cliente" }
    ];

    function Clientes(oController) {
        this.c             = oController;
    ...
    p.onSearch = function (oEvent) {
        ...
        var aF = sQ ? [new Filter({
            filters: [
                new Filter("kunnr", FilterOperator.Contains, sQ),
                new Filter("vkorg", FilterOperator.Contains, sQ),
                new Filter("vtweg", FilterOperator.Contains, sQ),
                new Filter("spart", FilterOperator.Contains, sQ),
                new Filter("vkbur", FilterOperator.Contains, sQ),
                new Filter("bran2", FilterOperator.Contains, sQ),
                new Filter("name1", FilterOperator.Contains, sQ)
            ],
            and: false
        })] : [];
    ...
    p.onEditar = function (oEvent) {
        ...
        this._getDlgEditar().then(function () {
            ...
            Fragment.byId(sId, "eCliKunnr").setValue(oData.kunnr  || "");
            Fragment.byId(sId, "eCliVkorg").setValue(oData.vkorg  || "");
            Fragment.byId(sId, "eCliVtweg").setValue(oData.vtweg  || "");
            Fragment.byId(sId, "eCliSpart").setValue(oData.spart  || "");
            Fragment.byId(sId, "eCliVkbur").setValue(oData.vkbur  || "");
            Fragment.byId(sId, "eCliBran2").setValue(oData.bran2  || "");
            Fragment.byId(sId, "eCliName1").setValue(oData.name1  || "");
            that._oDlgEditar.open();
        });
    };

    p.onConfirmarEditar = function () {
        var sId = this._sDlgEditarId;
        var oPayload = {
            vkorg: Fragment.byId(sId, "eCliVkorg").getValue().trim(),
            vtweg: Fragment.byId(sId, "eCliVtweg").getValue().trim(),
            spart: Fragment.byId(sId, "eCliSpart").getValue().trim(),
            vkbur: Fragment.byId(sId, "eCliVkbur").getValue().trim(),
            bran2: Fragment.byId(sId, "eCliBran2").getValue().trim(),
            name1: Fragment.byId(sId, "eCliName1").getValue().trim()
        };
        var that = this;
        MessageBox.confirm("¿Confirma la Edición del Registro?", {
            ...
            onClose : function (sAction) {
                ...
                that._model().update(that._sEditPath, oPayload, {
                    success: function () {
    ...
    p._getDlgEditar = function () {
        ...
        var sId = this.c.createId("cli_dlgEdit");
        this._sDlgEditarId = sId;
        return Fragment.load({ id: sId, controller: this.c,
            name: "zpncnd.datos.maestros.pncnddatosmaestros.view.fragment.DialogEditarClientes"
    ...
    p._getDlgMM = function () {
        ...
        var sId = this.c.createId("cli_dlgMM");
        this._sDlgMMId = sId;
        return Fragment.load({ id: sId, controller: this.c,
    ...
    p._getDlgCM = function () {
        ...
        var sId = this.c.createId("cli_dlgCM");
        this._sDlgCMId = sId;
        return Fragment.load({ id: sId, controller: this.c,
    ...
    p._getDlgCP = function () {
        ...
        var sId = this.c.createId("cli_dlgCP");
        this._sDlgCPId = sId;
        return Fragment.load({ id: sId, controller: this.c,
